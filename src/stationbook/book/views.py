...
class NetworkDetailsListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["network_doi"] = self.get_network_doi(
            self.kwargs.get("network_code"), self.kwargs.get("network_start_year")
        )
...
class StationDetailsListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        channel_url = node_wrapper.build_url_station_channel_level().format(
            self.station.fdsn_network.code, self.station.code
        )
        ...
        scg = fdsn_cha_manager.discover_station_channels(
            network_pk=self.station.fdsn_network.pk, station_pk=self.station.pk
        )
...
class ExtBasicDataUpdateView(StationUpdateViewBase):
    ...
    def get_object(self):
        ...
        obj = get_object_or_404(
            ...
            station__fdsn_network__start_date__year=self.kwargs.get(
                "network_start_year"
            ),
...
class ExtBoreholeDataUpdateView(StationUpdateViewBase):
    def __init__(self):
        StationUpdateViewBase.__init__(self, model=ExtBoreholeData, fields=("depth",))
...
@login_required
@transaction.atomic
def station_borehole_layer_add(
    request, network_code, network_start_year, station_code, station_start_year
):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            StationBookHelpers.add_ext_access_data(
                request.user,
                station,
                "Added borehole layer ({0})".format(borehole_layer.description),
            )
        ...
        return render(
            request,
            "station_borehole_layer_add.html",
...
@login_required
@transaction.atomic
def station_borehole_layer_edit(
    request,
    network_code,
    network_start_year,
    station_code,
    ...
    if request.method == "POST":
        ...
        StationBookHelpers.add_ext_access_data(
            request.user,
            station,
            "Edited borehole layer ({0})".format(borehole_layer.description),
        )
        ...
        return render(
            request,
            "station_borehole_layer_edit.html",
...
@login_required
@transaction.atomic
def station_borehole_layer_remove(
    request,
    network_code,
    network_start_year,
    station_code,
    ...
    if request.method == "POST":
        ...
        StationBookHelpers.add_ext_access_data(
            request.user,
            station,
            "Removed borehole layer ({0})".format(borehole_layer.description),
        )
        ...
        return render(
            request,
            "station_borehole_layer_remove.html",
            {"station": station, "layer": borehole_layer, "form": form},
        )
...
def station_photo_upload(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            StationBookHelpers.add_ext_access_data(
                request.user, station, "Added photo ({0})".format(photo.description)
            )
        ...
        return render(request, "upload_photo.html", {"station": station, "form": form})


@login_required
@transaction.atomic
def station_photo_edit(
    request,
    network_code,
    network_start_year,
    station_code,
    ...
    if request.method == "POST":
        ...
        StationBookHelpers.add_ext_access_data(
            request.user, station, "Photo edited ({0})".format(photo.description)
        )
        ...
        return render(
            request,
            "station_gallery_photo_edit.html",
...
def station_photo_remove(
    ...
    if request.method == "POST":
        ...
        StationBookHelpers.add_ext_access_data(
            request.user, station, "Removed photo ({0})".format(photo.description)
        )
        ...
        return render(
            request,
            "station_gallery_photo_remove.html",
            {"station": station, "img": photo, "form": form},
        )
...
class UserDetailsListView(ListView):
    ...
    def get_queryset(self):
        try:
            queryset = User.objects.get(username=self.kwargs.get("username"))
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["activity"] = ExtAccessData.objects.order_by("-updated_at").filter(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if network_class and network_class == "permanent":
                data = data.filter(fdsn_network__code__regex=r"[A-Z]{2}")
                search_phrase += "Network class: {}, ".format(network_class)

            if network_class and network_class == "temporary":
                data = data.filter(fdsn_network__code__regex=r"\d[A-Z]{1}")
                search_phrase += "Network class: {}, ".format(network_class)

            if network_access and network_access == "restricted":
                data = data.filter(fdsn_network__restricted_status="closed")
                search_phrase += "Network access: {}, ".format(network_access)

            if network_access and network_access == "unrestricted":
                data = data.filter(fdsn_network__restricted_status="open")
                search_phrase += "Network access: {}, ".format(network_access)
            ...
            if station_status and station_status == "open":
                data = data.filter(end_date__isnull=True)
                search_phrase += "Station status: {}, ".format(station_status)

            if station_status and station_status == "closed":
                data = data.filter(end_date__isnull=False)
                search_phrase += "Station status: {}, ".format(station_status)

            if station_access and station_access == "restricted":
                data = data.filter(restricted_status="closed")
                search_phrase += "Station access: {}, ".format(network_access)

            if station_access and station_access == "unrestricted":
                data = data.filter(restricted_status="open")
                search_phrase += "Station access: {}, ".format(network_access)
            ...
            if basin_flag:
                data = data.filter(ext_morphology_data__basin_flag=basin_flag)
                search_phrase += "Basin flag: {}, ".format(basin_flag)

            if vs30_from:
                data = data.filter(ext_morphology_data__vs_30__gte=vs30_from)
                search_phrase += "Vs 30 min: {}, ".format(vs30_from)

            if vs30_to:
                data = data.filter(ext_morphology_data__vs_30__lte=vs30_to)
                search_phrase += "Vs 30 max: {}, ".format(vs30_to)

            if f0_from:
                data = data.filter(ext_morphology_data__f0__gte=f0_from)
                search_phrase += "f0 min: {}, ".format(f0_from)

            if f0_to:
                data = data.filter(ext_morphology_data__f0__lte=f0_to)
                search_phrase += "f0 max: {}, ".format(f0_to)

            return render(
                request,
                "search_results.html",
                {"data": data, "search_phrase": search_phrase},
            )
        else:
            return render(request, "search_advanced.html", {"form": form})
    else:
