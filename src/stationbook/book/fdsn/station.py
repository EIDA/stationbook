    ...
    def get_network_station_graph(self):
        ...
        try:
            for network in root.findall(
                ...
                network_description = network.find(
                ...
                network_graph_network.restricted_status = (
                    network_restricted_status or 'unknown')
                ...
                for station in network.findall(
                    ...
                    station_site_name = station.find(
                        './/mw:Name', namespaces=self.NSMAP).text
                    station_latitude = station.find(
                        './/mw:Latitude', namespaces=self.NSMAP).text
                    station_longitude = station.find(
                        './/mw:Longitude', namespaces=self.NSMAP).text
                    station_elevation = station.find(
                        './/mw:Elevation', namespaces=self.NSMAP).text
                    station_creation_date = station.find(
                    ...
                    network_graph_network_station.restricted_status = (
                        station_restricted_status or 'unknown')
                    ...
                    network_graph_network.stations.append(
                        network_graph_network_station)
        
            return network_graph
        except (ConnectTimeout, HTTPError, ReadTimeout,
                Timeout, ConnectionError):
            StationBookLogger(__name__).log_exception(NetworkStationGraph.__name__)
            ...
            for station in network.stations:
                ...
                if FdsnStation.objects.filter(code=station.code).exists():
                    ...
                    try:
                        ...
                        StationBookLogger(__name__).log_exception(_refresh_station.__name__)
    except:
        StationBookLogger(__name__).log_exception(_refresh_station.__name__)
