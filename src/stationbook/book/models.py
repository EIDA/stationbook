...
class ExtOwnerData(ExtEntityBase):
    ...
    agency = models.CharField(max_length=STRING_LENGTH_SHORT, blank=True, default="n/a")
    city = models.CharField(max_length=STRING_LENGTH_SHORT, blank=True, default="n/a")
    street = models.CharField(max_length=STRING_LENGTH_SHORT, blank=True, default="n/a")
    country = models.CharField(
        max_length=STRING_LENGTH_SHORT, blank=True, default="n/a"
    )
    phone = models.CharField(max_length=STRING_LENGTH_SHORT, blank=True, default="n/a")
    email = models.CharField(max_length=STRING_LENGTH_SHORT, blank=True, default="n/a")
...
class FdsnNetwork(models.Model):
    fdsn_node = models.ForeignKey(
        FdsnNode, related_name="fdsn_networks", on_delete=models.CASCADE, default=None
    )
    ...
    def __str__(self):
        return "Node {0} Network {1} Year {2}".format(
            self.fdsn_node.code, self.code, self.start_date.year
        )
...
class FdsnStation(models.Model):
    ...
    start_date = models.DateTimeField(max_length=STRING_LENGTH_SHORT, blank=True)
    ...
    ext_basic_data = models.OneToOneField(
        ExtBasicData, related_name="station", null=True, on_delete=models.SET_NULL
    )
    ext_owner_data = models.OneToOneField(
        ExtOwnerData, related_name="station", null=True, on_delete=models.SET_NULL
    )
    ext_morphology_data = models.OneToOneField(
        ExtMorphologyData, related_name="station", null=True, on_delete=models.SET_NULL
    )
    ext_housing_data = models.OneToOneField(
        ExtHousingData, related_name="station", null=True, on_delete=models.SET_NULL
    )
    ext_borehole_data = models.OneToOneField(
        ExtBoreholeData, related_name="station", null=True, on_delete=models.SET_NULL
    )
    ...
    def get_created_date(self):
        return "{0}/{1}/{2}".format(
            self.creation_date.year, self.creation_date.month, self.creation_date.day
        )
...
class ExtAccessData(ExtEntityBase):
    fdsn_station = models.ForeignKey(
        FdsnStation, null=True, related_name="access_data", on_delete=models.SET_NULL
    )
    ...
    def __str__(self):
        return "{0} has been updated at {1} by {2}: {3}".format(
            self.fdsn_station.code, self.updated_at, self.updated_by, self.description
        )
...
class Photo(ExtEntityBase):
    ...
    fdsn_station = models.ForeignKey(
        FdsnStation, null=True, related_name="photos", on_delete=models.SET_NULL
    )
    ...
    def __str__(self):
        return "Station {0}, photo description: {1}".format(
            self.fdsn_station.code, self.description
        )
...
class Link(models.Model):
    ...
    def __str__(self):
        return "{0} - {1}".format(self.url, self.description)


# Search helper models
class SearchFdsnStationModel(models.Model):
    network_code = models.CharField(max_length=STRING_LENGTH_SHORT, blank=True)

    station_code = models.CharField(max_length=STRING_LENGTH_SHORT, blank=True)
    ...
    f0_from = models.DecimalField(
        max_digits=F0_INTEGERS + F0_DECIMALS, decimal_places=COORD_DECIMALS, blank=True
    )
